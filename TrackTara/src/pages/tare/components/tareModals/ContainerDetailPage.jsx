...
const ContainerDetailPage = () => {
  ...
  const containerHistory = useSelector(
    (state) => state.containerHistory?.histories || []
  );
  ...
  const getTypeName = (typeId) => {
    const type = containerTypes.find(
      (containerType) => containerType.id === typeId
    );
  ...
  if (!container)
    return <div className="alert alert-warning">Контейнер не знайдено</div>;
  ...
  return (
    <Container className="mt-5">
      ...
      >
        ...
        <Modal.Body>
          <p>Ви впевнені, що хочете видалити продукт із цього контейнера?</p>
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
            onClick={() => setShowRemoveProductModal(false)}
          >
            Скасувати
          </Button>
