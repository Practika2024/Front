...
} from "../../../store/state/actions/containerActions";
import {
  fetchContainerTypes,
  fetchContainerTypeNameById,
} from "../../../store/state/actions/containerTypeActions";
...
const ContainersTable = () => {
  ...
  const confirmSetProduct = () => {
    if (selectedProductId) {
      dispatch(
        addProductToContainer(selectedContainerId, selectedProductId)
      ).then(() => {
        fetchContainerData();
      ...
      for (const container of containers) {
        if (!names[container.typeId]) {
          const name = await dispatch(
            fetchContainerTypeNameById(container.typeId)
          );
  ...
  const currentItems = sortedContainers.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  return (
    <div className="container mt-5">
            ...
            <Table striped bordered hover responsive>
              ...
              <tbody>
                {currentItems.map((container) => (
                  <tr key={container.id}>
                    <td>
                      <a href={`/tare/detail/${container.id}`}>
      ...
      <Modal show={showProductModal} onHide={() => setShowProductModal(false)}>
        ...
        <Modal.Body>
          <p>Оберіть продукт для додавання в контейнер:</p>
          <Select
      ...
      <Modal
        ...
        onHide={() => setShowConfirmClearModal(false)}
      >
        <Modal.Header closeButton>
